...
- **📁 Strict Subfolders Restriction**: Optionally restrict sorting strictly to pre-existing subfolders (e.g. `g/ai`, `g/linux`, or a flat `happy/`, `sad/`, `smug/` reaction library). In strict mode MemeDrawer never creates any new directory — files that don't match an existing folder simply stay in the root.
